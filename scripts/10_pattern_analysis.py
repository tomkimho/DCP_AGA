...
def load_literature_data():
    ...
    if not filepath:
        print(f"ERROR: AGA 데이터 파일을 찾을 수 없습니다.")
        print(f"  검색 경로: {candidates}")
        return None
...
def build_network_analysis(df):
    ...
    target_col = _find_col(df, ['타겟(Target)', '주요_타겟', 'targets', '타겟'])
    compound_col = _find_col(df, ['화합물(Compound)', '분석된_화합물', 'compounds', '화합물'])
    pathway_col = _find_col(df, ['신호전달경로', '경로', 'pathways', 'pathway'])
    moa_col = _find_col(df, ['기전(MoA)', '기전', 'mechanism'])
    relevance_col = _find_col(df, ['관련도(1-5)', '관련도', 'relevance_score'])

    print(f"  컬럼 매핑: target={target_col}, compound={compound_col}, pathway={pathway_col}")

    for idx, row in df.iterrows():
        # Target 추출 및 정규화
        target_list = []
        raw_target = row.get(target_col, '') if target_col else ''
        ...
        # Compound 추출
        compound_list = []
        raw_compound = row.get(compound_col, '') if compound_col else ''
        if pd.notna(raw_compound) and str(raw_compound).strip():
            compound_str = str(raw_compound)
            for c in compound_str.replace(';', ',').split(','):
                c = c.strip()
                if c:
                    compound_list.append(c)

        # Pathway 추출
        raw_pathway = row.get(pathway_col, 'Unknown') if pathway_col else 'Unknown'
        pathway = str(raw_pathway).strip() if pd.notna(raw_pathway) else 'Unknown'

        # MoA 추출
        moa = str(row.get('작용기전', 'Unknown')).strip() if pd.notna(row.get('작용기전')) else 'Unknown'

        # Relevance 추출
        raw_rel = row.get(relevance_col, 0.5) if relevance_col else 0.5
        try:
            relevance = float(raw_rel) if pd.notna(raw_rel) else 0.5
