...
# Claude API 키 (Streamlit Cloud 배포 시 secrets에서 / 로컬에서는 02_정보추출.py에서 가져옴)
def load_api_key():
    ...
    try:
        ...
        pass
    # 2순위: 로컬 파일에서 읽기
    script_path = os.path.join(BASE_FOLDER, "scripts", "02_정보추출.py")
...
if df is None:
    st.error("데이터가 없습니다. 02_정보추출.py를 먼저 실행하세요.")
    st.stop()
...
# ============================================================
# 탭 7: 🏢 Control Center (픽셀 아트 가상 사무실)
# ============================================================
with tab7:
    ...
    if collection_log:
        ...
        st.markdown("""
        **시작하는 방법:**
        1. 로컬에서: `python scripts/08_자동처리_오케스트레이터.py`
    ...
    with st.expander("⚙️ 자동화 설정 안내"):
        ...
        **GitHub Actions로 매일 자동 실행하려면:**
        ...
        1. GitHub repo (DCP_AGA)에 아래 파일들을 업로드하세요:
           - `scripts/05_pubmed_자동수집.py`
           - `scripts/06_특허_자동수집.py`
           - `scripts/07_biorxiv_수집.py`
           - `scripts/08_자동처리_오케스트레이터.py`
